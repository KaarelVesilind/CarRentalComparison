export default class ElmoPriceCalculator {
    ...
    calculatePrice(car, searchParamsObj) {
        ...
        if (searchParamsObj.days >= 1) {
            if (
                searchParamsObj.days * price.day + searchParamsObj.hours * price.hour + searchParamsObj.minutes * price.minute >
                price.week
            ) {
        ...
        if (searchParamsObj.hours >= 1) {
            if (searchParamsObj.hours * price.hour + searchParamsObj.minutes * price.minute > price.day) {
                daysCost += price.day;
                searchParamsObj.hours = 0;
                searchParamsObj.minutes = 0;
            } else {
